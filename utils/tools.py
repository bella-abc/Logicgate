...
# RemoveacceleratorParameters
    ...
    if epoch in lr_adjust .keys ():
        ...
        for param_group in optimizer .param_groups :
            param_group ['lr']=lr 
        if printout :
            print ('Updated learning rate {}'.format (lr ))
...
class EarlyStopping :
    ...
    def save_checkpoint (self ,val_loss ,model ,path ):
        if self .verbose :
            print (f'Verification loss reduced ({self .val_loss_min :.6f} --> {val_loss :.6f})。Save model...')

        model =torch .save (model .state_dict (),path +'/'+'checkpoint')
...
def vali (args ,model ,vali_data ,vali_loader ,criterion ,mae_metric =None ):
    total_loss =[]
    total_mae_loss =[]
    model .eval ()
    ...
    with torch .no_grad ():
        ...
        # Handle different data loader outputs based on model type
            if hasattr (args ,'model')and is_logicgate_model (args .model )and len (batch_data )==5 :
